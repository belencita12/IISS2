describe("Formulario de Edición de Empleados", () => {
    ...
    it("Debe mostrar errores al borrar campos requeridos y enviar", () => {
        ...
        cy.get('input[placeholder="Ingrese el correo electrónico"]').clear();
    ...
    it("Debe mostrar error si se ingresa un RUC con formato inválido", () => {
        cy.get('input[placeholder="Ingrese el RUC"]').clear().type("12345");
        cy.get('input[placeholder="Ingrese el número de teléfono"]').clear().type(NEW_PHONE);
        cy.get("button").contains("Actualizar").click();
        cy.wait(5000);
    ...
    /*it("Debe mostrar error si se ingresa un RUC ya registrado", () => {
        ...
        cy.get('input[placeholder="Ingrese el RUC"]').clear().type(RUC_EXISTENTE);
        cy.get('input[placeholder="Ingrese el número de teléfono"]').clear().type(NEW_PHONE);
        cy.get('input[placeholder = "Ingrese la dirección"]').clear().type("El país de las maravillas");
        cy.get("button").contains("Registrar").click();
        cy.wait(6000);
    ...
    it("Debe mostrar error si se ingresa un email ya registrado", () => {
        cy.get('input[placeholder="Ingrese el correo electrónico"]').clear().type(EMAIL_EXISTENTE);
        cy.get('input[placeholder="Ingrese el número de teléfono"]').clear().type(NEW_PHONE);
        cy.get('input[placeholder = "Ingrese la dirección"]').clear().type("El país de las maravillas");
        cy.get("button").contains("Actualizar").click();
        cy.wait(5000);
    ...
    it("Debe actualizar correctamente", () => {
        cy.get('input[placeholder="Ingrese el correo electrónico"]').clear().type(NEW_EMAIL);
        cy.get('input[placeholder="Ingrese el número de teléfono"]').clear().type(NEW_PHONE);
        cy.get('input[placeholder = "Ingrese la dirección"]').clear().type("El país de las maravillas");
        cy.get("button").contains("Actualizar").click();
        cy.wait(5000);
