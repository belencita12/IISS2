...
const ListView = ({ pets }: { pets: IPet[] }) => (
    <div className="flex flex-col space-y-4 pb-10">
        {pets.map((pet) => (
            <div key={pet.id} className="flex items-center gap-4 p-4 border rounded-lg shadow-sm hover:bg-gray-100 transition">
                <Avatar className="w-16 h-16 overflow-hidden rounded-full">
                    <AvatarImage src={pet.profileImg || IMAGE_NOT_FOUND} alt={pet.name || 'Imagen no encontrada'} />
                    <AvatarFallback>{pet.name.charAt(0)}</AvatarFallback>
                </Avatar>
                <div className="flex-1">
                    <h3 className="text-lg font-semibold">{pet.name}</h3>
                    <p className="text-gray-600">{pet.species.name} - {pet.race.name}</p>
                    <p className="text-sm text-gray-500">{(pet.sex === 'M' ? 'Macho' : 'Hembra')}</p>
                </div>
...
const PetList = ({ userId, token }: { userId: number, token: string }) => {
    ...
    useEffect(() => {
        const fetchPets = async () => {
            ...
            try {
                ...
                if (jsonResponse.total === 0) throw new Error('Error al obtener mascotas');
    ...
    const petListView = useMemo(() => 
        isGridView ? <GridView pets={filteredPets} /> : <ListView pets={filteredPets} />
