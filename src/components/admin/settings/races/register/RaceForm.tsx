...
export const RaceForm = ({
  ...
  const {
    ...
    defaultValues: { name: "", speciesId: 0 },
  });

  useEffect(() => {
    const fetchSpeciesAndReset = async () => {
      if (isOpen) {
        try {
    ...
    fetchSpeciesAndReset();
  }, [token, isOpen, initialData, reset]);
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          {errors.speciesId && (
            <p className="text-red-500">{errors.speciesId.message}</p>
          )}
        </div>
