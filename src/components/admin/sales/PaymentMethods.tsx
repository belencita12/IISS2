...
import { PaymentMethod, PaymentMethodResponse } from "@/lib/sales/IPaymentMethod";
...
export default function PaymentMethods({
  ...
  const { data, get, loading } = useFetch<PaymentMethodResponse>(
    "",
    token
  );
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data.data.length > 0) onSelectedMethodChange(data.data[0].id.toString());
    }
  }, [data]);
