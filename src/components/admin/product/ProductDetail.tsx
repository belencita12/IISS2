...
export default function ProductDetail({ token }: ProductDetailProps) {
  ...
  const { delete: deleteReq, loading: isDelLoading } = useFetch<void, null>(PRODUCT_API, token, { method: "DELETE" })
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <div className="md:col-span-3 flex justify-center">
          {product.image?.originalUrl ? (
            ...
            <div className="w-40 h-40 bg-gray-200 flex items-center justify-center text-4xl font-bold rounded-md">
        ...
        <div className="md:col-span-9">
          <ProductInfo product={product} stockDetails={stockDetails} isStockLoading={isLoading} />
        </div>
      </div>

      {/* Botones */}
      <div className="flex gap-4 mt-6 justify-center">
        <Button variant="outline" onClick={() => setIsDeleteModalOpen(true)} className="px-6">
          Eliminar
        </Button>
      ...
      <div className="mt-4">
        <h3 className="text-2xl font-semibold text-center mb-6">Cantidad por Sucursales</h3>
        <StockList stockDetails={stockDetails} stocks={stocks} isLoading={isLoading} />
      </div>
