    ...
    <div className="space-y-4">
      <div className="text-center">
        {isLoading ? (
          <div className="flex items-center justify-center gap-2 mt-4">
            ...
            <h2 className="text-sm text-gray-600">{t("button.loading")}</h2>
          </div>
        ) : (
      ...
      {error && (
        <div className="text-center text-red-500 mt-4">
          {t("error.errorLoadProviderDetails")} {error.message}
        </div>
      )}
      ...
      {!isLoading && !error && provider ? (
        <div className="space-y-4">
          <div className="pl-2">
            <p className="text-sm text-gray-600">{t("providers.details.description")}</p>
            <p className="font-medium">{provider.description}</p>
          </div>

          <div className="border rounded-lg p-2 pl-2">
            <p className="text-sm text-gray-600">{t("providers.details.contact")}</p>

            <div className="mt-2 flex items-center space-x-3">
              <Phone size={20} className="text-black-600" />
              <div>
                <p className="text-xs text-gray-500">{t("providers.details.phone")}</p>
            ...
            <div className="mt-4 flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs text-gray-500">{t("providers.details.ruc")}</p>
